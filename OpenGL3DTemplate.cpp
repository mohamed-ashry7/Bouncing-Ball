...
void drawBrick(int x , int y , int z, int whichPlane) { // 0 ->XY , 1 ->XZ 
	...
	*/
	...
	if (whichPlane == 0) {
		if (z == 0) {
			glTranslated(x + 1, y + 1, -scale); // big cube at (0.5, 0.5, 0.5)
			glScaled(1, 1, scale);
			glutSolidCube(BRICK_SIZE);
		}
		else {
			glTranslated(x + 1, y + 1, ROOM_SIZE+ scale); // big cube at (0.5, 0.5, 0.5)
			glScaled(1, 1, scale);
			glutSolidCube(BRICK_SIZE);
		}
	}
		...
		if (y==0) {
			glTranslated(x + 1, -scale, z+1); // big cube at (0.5, 0.5, 0.5)
			glScaled(1, scale, 1);
			glutSolidCube(BRICK_SIZE);
		}
		else {
			glTranslated(x + 1, ROOM_SIZE+ scale, z + 1); // big cube at (0.5, 0.5, 0.5)
			glScaled(1, scale, 1);
...
void drawWalls() {
	...
	for (int i = 0; i < ROOM_SIZE; i += BRICK_SIZE) {
		for (int j = 0; j < ROOM_SIZE; j += BRICK_SIZE) {
			if (i == 2*BRICK_SIZE && j == 3*BRICK_SIZE) {
				glColor3f(0, 0, 0);
				W2.setCell(i / BRICK_SIZE, j / BRICK_SIZE, 100);
				glPushMatrix();
				drawBrick(i, 0, j, 1);
				glPopMatrix();
				continue; 
...
void showGameInfo() {
	...
	print(ROOM_SIZE, ROOM_SIZE, ROOM_SIZE,"Controllers for directions : w -> up , s -> down , d -> right , a -> left " );
	print(ROOM_SIZE, ROOM_SIZE-1, ROOM_SIZE, "Controllers for Camera : fixed mode -> h , simple mode -> j , enhanced mode -> k ");
